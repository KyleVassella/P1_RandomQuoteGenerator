/***********************************************************
This project was tested on Google Chrome, Safari, Firefox & IE
***********************************************************/

document.getElementById('load-quote').addEventListener("click", printQuote, false); // event listener to respond to clicks on the page. When user clicks anywhere on the page, the printQuote() function is called.
...
var quotes = [ // an array of quote objects, one of which will display its values when printQuote() is called.
  {
    quote: "A man away from home need feel no shame.",
    source: "Mozart",
    year: "1780",
    tag: "| inspirational",
    seen: false
  }, {
    quote: "Don't train alone - it only embeds your errors.",
    source: "Giralt of Rivia",
    citation: "The Witcher 3",
    tag: "| gaming",
    seen: false
  }, {
    ...
    quote: "That's the thing that you see about great business owners..is this...undying optimism. They're realistic, but they're optimistic. Ooh, that rhymes!",
    ...
    quote: "My dad is the greatest man alive. If anyone disagrees, we'll go outside.",
    source: "Jeb Bush",
    tag: "| politics",
    seen: false
  }, {
...
function myClearInterval() { //  resets the 15 second countdown every time the load-quote button is pressed - this ensures that the 15 second countdown stays consistent (restarts) each time the load-quote button is pressed.
...
function printQuote() { // calls the getRandomQuote() function and stores the returned quote object in a variable (var quoteObj). Constructs a string using the different properties of the quote object. Manipulates the DOM to display the final HTML string ot the page.
  ...
  myClearInterval(); //calls the clearInterval function which resets the 15 second countdown each time the load-quote button is pressed, improving UX.
